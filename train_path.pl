...
location(arl_ramkhamhaeng, 13.7425102, 100.5995472).
location(arl_hua_mak, 13.738068, 100.6371115).
...
location(mrt_blue_tao_poon, 13.8070074, 100.5281396).
/* MRT Purple */
...
astar1([(_,_,Tp,[Final|R])|_],Final,[Final|R],Tp,Result):- reverse([Final|R],Result),write('Path = '),
                          write(L3).
...
/* Haversine Formula to calculate min path, also to find time */
 estimation(Station1,Station2,Est):- location(Station1,Lat1,Lng1), location(Station2,Lat2,Lng2),
                   ...
                   LngRad2 is (Lng2*pi)/180,
                   R is 6371e3, /* in metres */
                   DeltaLat is LatRad2 - LatRad1,
                   DeltaLng is LngRad2 - LngRad1,
                   A1 is sin(DeltaLat/2)^2,
                   A2 is cos(LatRad1) * cos(LatRad2),
                   A3 is sin(DeltaLng/2)^2,
                   A is A1 + A2 * A3,
